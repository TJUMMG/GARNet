...
def build_file_paths(base, group_names=None, file_names=None, suffix='.png'):
    if file_names == None and group_names == None:
        ...
        for group_name in listdir(base):
            ...
            group_file_names = listdir(group_path) # type: list
            ...
            indices.append(cur_group_end_index) # 每一组结束时的个数
        ...
        return file_paths, group_names, file_names, indices # 返回文件的全路径，哪一个组的名字，文件名，计数值。共返回4个列表，是整个数据集的
    else:
        file_paths = list(map(lambda i: join(base, group_names[i], file_names[i] + suffix), range(len(file_names))))
        return file_paths # 返回文件的全路径，可用来返回gt的路径，不需要再去遍历文件夹
...
def random_flip(img, gt):
    datas = (img, gt)
    if random.random() > 0.5: # 生成（0，1）的浮点数 # map()根据提供的函数对指定的序列做映射 # lamda 形参列表：表达式 （简化代码）
        datas = tuple(map(lambda data: transforms.functional.hflip(data) if data is not None else None, datas))
    return datas


class ImageData(data.Dataset): # request我需要传进什么类型的数据，原图、gt、sisms、、
    def __init__(self, roots, request, aug_transform=None, rgb_transform=None, gray_transform=None):
        ...
        self.img_paths = img_paths # 返回各种数据的路径
    ...
    def __getitem__(self, item):
        ...
        if self.aug_transform is not None:
            img, gt = self.aug_transform(img, gt) #返回元组中的每一个元素
...
class Cosal_Sampler(data.Sampler):
    def __init__(self, indices, shuffle, batch_size):
        self.indices = indices # dataset.indices
    ...
    def reset_batches_indices(self):
        ...
        for end_idx in self.indices: # [23, 34, ..., ]
            ...
            idx = 0
            while idx < num:# 如果batch_size为空的话，batch_size的值就为group的数目
                batch_size = num if self.batch_size == None else self.batch_size
                batch_indices = group_indices[idx:idx + batch_size] #虽然可能会超出索引长度，但超过的地方为空
        ...
        if self.shuffle: # 对所有组的进行shuffle，但一个batch来自同一个组
    ...
    def __iter__(self):
        if self.shuffle:
            self.reset_batches_indices()
        return iter(self.batches_indices) #返回一个迭代器

    def __len__(self):
        return self.len # 有多少个batch
...
def get_loader(roots, request, batch_size, data_aug, shuffle, num_thread=4, pin=True):
    ...
    data_loader = data.DataLoader(dataset=dataset, batch_sampler=cosal_sampler, num_workers=num_thread, pin_memory=pin)#batch_sampler定义读取数据的方式, pin_memory 锁住内存
    return data_loader